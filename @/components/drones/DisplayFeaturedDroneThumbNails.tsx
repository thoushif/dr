...
function DisplayFeaturedDroneThumbNails({ drones }: Props) {
  const { selectedDrones, removeDroneFromCompare } = useDroneCompare();
  return (
    <div>
      <div className="grid grid-cols-1 gap-6 px-8 pb-24 md:grid-cols-4 gap-y-6">
        {drones &&
          drones.map((dr) => (
...
export default DisplayFeaturedDroneThumbNails;
